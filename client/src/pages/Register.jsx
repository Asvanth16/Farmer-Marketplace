...
function Register() {
    ...
    return (
        <div className="flex min-h-screen w-full bg-gradient-to-r from-emerald-950 via-brand-800 to-emerald-300 m-0 p-0 box-border overflow-x-hidden overflow-y-auto antialiased select-none relative">
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-5 sm:p-10 md:p-10 lg:p-12 xl:p-16 shrink-0 relative bg-transparent z-10">
                
                {/* MOBILE & TABLET HEADER ONLY */}
                <div className="flex lg:hidden items-center gap-3 mb-8 text-white relative z-10 animate-fade-in">
                    <div className="p-2 bg-white/10 backdrop-blur-md rounded-xl border border-white/10 shadow-inner">
                        <Leaf className="h-7 w-7 text-brand-100 fill-brand-100/20" />
                    </div>
                ...
                {/* Increased width baseline from max-w-sm to max-w-[92%] on small touchscreens, and expanded padding metrics */}
                <div className="w-[94%] max-w-md sm:max-w-md md:max-w-md lg:max-w-lg xl:max-w-xl bg-white border border-slate-100/80 p-6 sm:p-10 md:p-8 lg:p-10 xl:p-12 rounded-2xl shadow-2xl shadow-emerald-950/10 space-y-6 md:space-y-6 lg:space-y-8 relative z-10">
                    ...
                    <div className="space-y-2.5 md:space-y-2">
                        ...
                        <h2 className="text-2xl sm:text-3xl lg:text-3xl font-black text-earth-900 tracking-tight">Create Account</h2>
                        <p className="text-slate-500 text-sm font-medium">Set up your profile to start exploring listings.</p>
                    </div>

                    {error && (
                        <div className="p-4 bg-red-50 border-l-4 border-red-500 text-red-700 text-sm md:text-base font-semibold rounded-r-xl shadow-xs animate-shake">
                            {error}
                        </div>
                    )}

                    <form onSubmit={handleSubmit} className="space-y-5 md:space-y-5 lg:space-y-6">
                        
                        {/* Full Name Field */}
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-earth-800 tracking-wide block">Full Name</label>
                            <div className="relative group">
                                <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-brand-500 transition-colors" />
                                <input
                                    type="text"
                                    required
                                    value={name}
                                    ...
                                    className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-earth-900 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all text-sm font-medium shadow-xs"
                                />
                            </div>
                        </div>
                        ...
                        {/* Email Field */}
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-earth-800 tracking-wide block">Email Address</label>
                            <div className="relative group">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-earth-800 tracking-wide block">Select Portal Role</label>
                            <div className="grid grid-cols-2 gap-3.5">
                                <button
                                    type="button"
                                    onClick={() => handleRoleChange("farmer")}
                                    className={`py-3 px-4 rounded-xl text-sm font-bold tracking-wide border transition-all flex items-center justify-center gap-2 cursor-pointer ${role === 'farmer'
                                ...
                                <button
                                    ...
                                    className={`py-3 px-4 rounded-xl text-sm font-bold tracking-wide border transition-all flex items-center justify-center gap-2 cursor-pointer ${role === 'customer'
                                        ? 'bg-brand-50 border-brand-500 text-brand-700 shadow-md shadow-brand-500/5'
                                        : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                                        }`}
                                >
                        ...
                        {role === 'farmer' && (
                            <div className="space-y-5 animate-fade-in">
                                {/* Farm Location Field */}
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-earth-800 tracking-wide block">Farm Location</label>
                                    <div className="relative group">
                                        <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-brand-500 transition-colors" />
                                        <input
                                            type="text"
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-earth-800 tracking-wide block">Farming Method</label>
                                    <div className="relative group">
                                        <Sprout className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-brand-500 transition-colors pointer-events-none" />
                                        <select
                                            value={farmingMethod}
                                            onChange={(e) => setFarmingMethod(e.target.value)}
                                            className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-earth-900 focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all text-sm font-medium shadow-xs appearance-none"
                                        >
                        ...
                        {/* Password Field */}
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-earth-800 tracking-wide block">Password</label>
                            <div className="relative group">
                        ...
                        <div className="space-y-4 pt-3">
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-brand-500 hover:bg-brand-600 active:scale-[0.99] text-white font-bold rounded-xl shadow-md shadow-brand-500/10 hover:shadow-brand-600/20 transition-all focus:outline-none focus:ring-4 focus:ring-brand-500/20 disabled:opacity-50 text-base cursor-pointer tracking-wide"
                            >
                                {isLoading ? (
                            ...
                            <div className="text-center text-sm font-medium text-slate-500 tracking-wide">
